...
\begin{folhadeaprovacao}
  ...
  \begin{center}
   ...
   \assinatura{\textbf{Prof. Antonio Tadeu, Ph.D.}}
...
\begin{resumo}[Abstract]
 ...
 Large-scale spatio-temporal simulations with quantified uncertainty enable scientists/decision-makers to make precise statements about the degree of confidence they have in their simulation-based predictions. This uncertainty could be quantified or characterized in different ways, from the use of low order statistical moments (the most used), to trying to evaluate the complete \textit{PDF} (the best way). Only the characterization of the uncertainty by using the \textit{PDF} allows aware decisions. The uncertainty also needs to be characterized in a way that allows researchers to answer queries that arise in the future.
 
 In this thesis, we propose a new method to quantify the uncertainty in large-scale spatio-temporal models using the Generalized Lambda Distribution (\textit{GLD}). We show how the use of the \textit{GLD} allow us to characterize the uncertainty on each spatio-temporal location in a way that can be used latter to answer questions that arise in the \textit{UQ} context. To ilustrate this, we answer the following questions: (\textit{i}) how to group the output of the \textit{UQ} process based in the simillarity of the uncertainty?, (\textit{ii}) what is the uncertainty in some spatio-temporal locations not analysed previously?, (\textit{iii}) what is the uncertainty of an specific spatio-temporal region?, (\textit{iv}) how to compare two regions as a function of its uncertainty?, and (\textit{v}) what is the less uncertain model from a set of model, to predict some quantity of interest in an spatio-temporal region?
 
The method was tested in realistic use cases from various scientific areas.
...
\include{capitulos/applicability}
