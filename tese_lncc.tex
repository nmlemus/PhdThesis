...
\begin{folhadeaprovacao}
  ...
  \begin{center}
   ...
   \assinatura{\textbf{Prof. Hugo de La Cruz, D.Sc.}}
   \assinatura{\textbf{Prof. Artur Ziviani, D.Sc.}}
...
\begin{dedicatoria}
   ...
   \textit{to my wife Mayte, \\
to my children Leandro and Claudia.\\} \vspace*{\fill}
\end{dedicatoria}
% ---
...
\begin{resumo}
As simulações espaço-temporais a grande escala com incerteza quantificada permitem que os cientistas avaliem com precisão o grau de confiança de suas previsões. Essa incerteza pode ser quantificada ou caracterizada de diferentes maneiras, desde o uso de momentos estatísticos de baixa ordem (o mais comumente usado) até a avaliação de um PDF completo (uma abordagem mais completa). Este último fornece uma descrição mais abrangente da incerteza que leva a decisões conscientes. No entanto, a estimação da PDFs é uma tarefa computacionalmente intensiva. Além disso, devido à heterogeneidade, a incerteza calculada nas regiões do conjunto de dados é dificultada pela representação com diferentes tipos de PDF.
 
Nesta tese, propomos um novo método para quantificar a incerteza em modelos espaço-temporais de larga escala baseados na Distribuição Generalizada de Lambda (GLD). O GLD é uma família de PDFs que modela bem a heterogeneidade da incerteza, conforme discutido acima. O GLD é especificado por 4 parâmetros o que simplifica as comparações das PDFs facilitando o processamento analítico, como clustering. Nós mostramos como o conjunto de dados modelado através de GLDs pode ser usado para responder a consultas, tais como: (\textit{i}) como agrupa a saída do processo de quantificação de incerteza com base na semelhança de incerteza?, (\textit{ii}) qual é a incerteza em algumas localizações espaço-temporais não analisadas anteriormente?, (\textit{iii}) qual é a incerteza de uma região espaço-temporal específica?, (\textit{iv}) como comparar duas regiões em função da sua incerteza?, e (\textit{v}) qual é o modelo menos incerto de um conjunto de modelos?
